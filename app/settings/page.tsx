...
import {
  ...
  UserCircle, UserPlus, Pencil, X, Plus, RotateCcw,
} from 'lucide-react';
...
export default function SettingsPage() {
  ...
  // Data recovery
  const [orphanedAccounts, setOrphanedAccounts] = useState<{ userId: string; healthFiles: number; uploadFiles: number }[]>([]);
  const [recoveryLoading, setRecoveryLoading] = useState(false);
  const [recoveryEmail, setRecoveryEmail] = useState('');
  const [recoveryName, setRecoveryName] = useState('');
  const [recoveryUserId, setRecoveryUserId] = useState('');
  const [recoveryResult, setRecoveryResult] = useState<{ tempPassword: string; healthFiles: number; uploadFiles: number } | null>(null);
  ...
  const handleScanOrphaned = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) setOrphanedAccounts(data.orphanedAccounts ?? []);
      else setRecoveryError(data.error ?? 'Scan failed.');
    } catch { setRecoveryError('Something went wrong.'); }
  ...
  const handleRestoreAccount = async (e: React.FormEvent) => {
    ...
    setRecoveryError('');
    setRecoveryResult(null);
    if (!recoveryEmail || !recoveryUserId) { setRecoveryError('Email and User ID are required.'); return; }
    setRecoveryLoading(true);
    try {
      const res = await fetch('/api/admin/recover', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: recoveryEmail, name: recoveryName, userId: recoveryUserId }),
      });
      const data = await res.json();
      if (res.ok) {
        ...
        setRecoveryName('');
        setRecoveryUserId('');
        setOrphanedAccounts(prev => prev.filter(o => o.userId !== recoveryUserId));
      } else {
        setRecoveryError(data.error ?? 'Restore failed.');
      }
    } catch { setRecoveryError('Something went wrong.'); }
    setRecoveryLoading(false);
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: '#F8FAFC' }}>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 pb-24 sm:pb-10">
        ...
        {/* ── Admin: Data Recovery ── */}
        {currentUser?.role === 'admin' && (
          <section className="bg-white rounded-2xl border border-orange-200 shadow-sm overflow-hidden mt-4">
            <div className="px-6 py-4 border-b border-orange-100">
              <h2 className="text-xs font-semibold text-orange-600 uppercase tracking-wide">Data Recovery</h2>
            </div>
            <div className="px-6 py-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-gray-900 mb-1">Restore Deleted Account</p>
                  <p className="text-sm text-gray-500 mb-4">
                    If a user account was removed but their health data remains in storage, you can re-create the account here. This only works when data was not wiped (admin deletion leaves data intact; self-deletion does not).
                  </p>

                  <button
                    onClick={handleScanOrphaned}
                    disabled={recoveryLoading}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 hover:bg-orange-100 text-orange-700 border border-orange-200 rounded-lg text-xs font-semibold transition-colors mb-4"
                  >
                    {recoveryLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RotateCcw className="w-3.5 h-3.5" />}
                  ...
                  {orphanedAccounts.length > 0 && (
                    <div className="mb-4 bg-orange-50 border border-orange-200 rounded-xl p-3">
                      <p className="text-xs font-semibold text-orange-800 mb-2">Found {orphanedAccounts.length} orphaned data set(s) — no active account:</p>
                      <div className="space-y-1">
                        {orphanedAccounts.map(o => (
                          <div key={o.userId} className="flex items-center justify-between gap-2 text-xs">
                            <span className="font-mono text-orange-700 truncate">{o.userId}</span>
                            <span className="text-orange-500 shrink-0">{o.healthFiles} data · {o.uploadFiles} files</span>
                            <button
                              onClick={() => setRecoveryUserId(o.userId)}
                              className="shrink-0 px-2 py-0.5 bg-orange-600 text-white rounded text-[10px] font-semibold hover:bg-orange-700"
                            >
                              Use
                            </button>
                          </div>
                  ...
                  {orphanedAccounts.length === 0 && !recoveryLoading && orphanedAccounts !== undefined && (
                    <div />
                  )}
                  ...
                  {recoveryResult && (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                      <div className="text-xs font-semibold text-green-800 mb-1">
                        Account restored — {recoveryResult.healthFiles} data files, {recoveryResult.uploadFiles} uploads recovered.
                      </div>
                      <div className="text-xs text-green-700 mb-1">Temporary password (share once):</div>
                      <div className="font-mono text-sm bg-white border border-green-300 rounded px-2 py-1 text-green-900 select-all">{recoveryResult.tempPassword}</div>
                    </div>
                  )}

                  {recoveryError && (
                    <div className="flex items-center gap-1.5 text-xs text-red-600 mb-3">
                      <XCircle className="w-3.5 h-3.5 shrink-0" />{recoveryError}
                    </div>
                  )}

                  <form onSubmit={handleRestoreAccount} className="space-y-2">
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">User ID (from scan above)</label>
                      <input
                        type="text"
                        ...
                        style={{ color: '#111827', backgroundColor: '#ffffff' }}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm font-mono focus:ring-2 focus:ring-orange-400 focus:border-orange-400 outline-none"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-600 mb-1">Email address to restore</label>
                      <input
                        type="email"
                        value={recoveryEmail}
                    ...
                    <button
                      type="submit"
                      disabled={recoveryLoading}
                      className="flex items-center gap-1.5 px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm font-medium disabled:opacity-40 transition-colors"
                    >
                      {recoveryLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RotateCcw className="w-3.5 h-3.5" />}
                      Restore Account
                    </button>
